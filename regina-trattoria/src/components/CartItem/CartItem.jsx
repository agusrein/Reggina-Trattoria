...
const CartItem = ({ item, count, deleteItem}) => {
  return (
    ...
    <div className='col-4 div__container--CartItem rounded-2 p-3 shadow-sm'>
      ...
      <div className='div__dividier rounded-pill'></div>
      <div className='col-12 d-flex'>
      <p className='mt-2 col-10'>Cantidad: {count}</p>
      <button className='col-2 rounded-pill btn__delete mt-2 shadow-sm' onClick={()=>deleteItem(item.index)}>Delete</button>
      </div>
      <h5>$ {item.price}</h5>
      </div>
      </div>
